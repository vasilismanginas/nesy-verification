...
class BoundLogSoftmaxImpl(nn.Module):
    """TODO EdS: Actually implement this"""
    ...
    def forward(self, x):
        ...
        softmax = x / s
        epsilon = 1e-12
        return torch.log(softmax.clamp(min=epsilon))


class BoundLogSoftmax(Bound):
    """TODO EdS: Actually implement this"""
    ...
    def forward(self, x):
        ...
        if self.option == 'complex':
            self.input = (x,)
            self.model = BoundLogSoftmaxImpl(self.axis)
            self.model.device = self.device
            return self.model(x)
        else:
            return F.log_softmax(x, dim=self.axis)  # TODO EdS: I think the 'complex' flag will need to be set as otherwise the log function will mean -inf
